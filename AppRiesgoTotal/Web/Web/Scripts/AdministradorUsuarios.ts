...
head.ready(function () {
    ...
    //variables Globalaes
    var permisosGlobal;
    var permisosAlmacenarGlobal: Array<PC_Permiso_Result>;
    ...
    if (UsuarioLogueado.CodigoGrupoAgentes == null || UsuarioLogueado.CodigoGrupoAgentes == undefined || UsuarioLogueado.CodigoGrupoAgentes == 0 || UsuarioLogueado.rol[0].Id == 1 || UsuarioLogueado.rol[0].Id == 3) {
        ...
        //opciones coberturas
        Loading_Hide(true);
        get$rol$CorredoresObtenerRoles(
            function (res: Msg) {
                //cargamos los permisos
                get$usuario$CorredoresObtenerPermisosActivos(
                    function (res: Msg) {

                        CargarGridUsuarios();
                        var tablaPermisos = $('#adminCoberturas');
                        var data = <Array<PC_Permiso_Result>>res.Datos;
                ...
                var data = <PC_Rol_Result>res.Datos;
    ...
    var lstUsuarios = new Array<PC_UsuarioRol_Result>();
    ...
    function CargarGridUsuarios() {
        ...
        get$usuario$CorredoresObtenerUsuariosActivosPorEmpresa(IdEmpresaFiltro,
            function (res: Msg) {
    ...
    function CargarGridUsuariosGrupo() {
        var data: Array<PC_UsuarioRol_Result> = new Array < PC_UsuarioRol_Result >()
        //recorremos los brokers
        for (let br of listaBrokers) {

            // LLENADO DE GRILLA DE SELECCIÓN DE LISTAS
            get$usuario$CorredoresObtenerUsuariosActivosPorEmpresa(br,
                function (res: Msg) {
                    //recorremos y vamos cargando la grid
                    var dataR = <Array<PC_UsuarioRol_Result>>res.Datos;
                    for (let d of dataR) {
                        data.push(d);
                    }
                    $('#gridUsuarios').data('kendoGrid').dataSource.data(data);
    ...
    $('#butGuardar').click(function () {
        ...
        var usuario: PC_UsuarioRol_Result = new PC_UsuarioRol_Result();

        permisosAlmacenarGlobal = new Array<PC_Permiso_Result>();

        for (let obj of permisosGlobal) {
            if ($('#check_' + obj.IDPermiso).prop('checked')) {
                var object: PC_Permiso_Result = new PC_Permiso_Result();
        ...
        var Roles = new Array<PC_Rol_Result>();
        ...
        //ws de guardado        
        post$usuario$CorredoresGrabarUsuarioCorp(usuario,
            function (res: Msg) {
                if (idUsuario !== undefined && idUsuario !== 0)
    ...
    $("#butEliminar").click(function () {
        ...
        Confirmation("Está seguro de eliminar el usuario seleccionado?",
            function () {
                if (idUsuario !== undefined && idUsuario !== 0) {
                    get$usuario$CorredoresEliminarUsuario(idUsuario,
                        function (res: Msg) {
                            if (res.Datos == true) {
    ...
    function RegenerarClave(e) {
        ...
        e.preventDefault();
        var dataItem = <PC_UsuarioRol_Result>this.dataItem($(e.currentTarget).closest("tr"));

        Confirmation("Esta acción enviará un correo electrónico al usuario con una nueva contraseña. Está seguro de proceder con el reseteo de clave?",
            function () {
                get$usuario$CorredoresResetearClaveUsuarioPorID(dataItem.Id,
                    function (res: Msg) {
                        var data = res.Datos;
                        if (data == true) {
    ...
    function EditarRegistro(e) {
        ...
        //lleno los check
        get$usuario$CorredoresObtenerPermisosUsuarioPorID(dataItem.Id,
            function (res: Msg) {
                var data = <Array<PC_Permiso_Result>>res.Datos;
                for (let d of data) {
