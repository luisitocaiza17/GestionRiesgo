...
head.ready(function () { 
    ...
    var IDEmpresa:number = 0;
    var IDSucursal: number = 0;
    var IDProducto: string = '';
    var IDNumeroContratoAfiliado = 0;
    var yoAfiliado: ContratoEntityList;
    var BeneficiariosIndividuales = Array<BeneficiarioEntity>();
    //inicializacion
    ...
    //Elementos Globales
    var listAfiliadosGlobal = new Array<ContratoEntityList>();
    var listEmpresaGlobal = new Array<EmpresaList>()
    ...
    function onSelect(e) {
        ...
        if (index === 1) {
            if (tipoProceso === 'Independiente' || tipoProceso == 'Afiliado' || tipoProceso === 'Experience' || tipoProceso === 'Oncocare') {
                ...
                if (yoAfiliado.CodigoProducto != 'COR' && yoAfiliado.CodigoProducto != 'SMAR'
                    ...
                    get$contrato$ConsultarDetalleCuotasXContrato
                        (IDNumeroContratoAfiliado, function (res: Msg) {
                            var tipoPago: DetalleFormaPagoEntity = <DetalleFormaPagoEntity>res.Datos;
                    ...
                    $('#BusquedaformaPagoIndependiente').show();
                    $('#BusquedaformaPagoEmpresa').hide();
                    get$facturacion$ObtenerFormaPagoEmpresa
                        (Number(yoAfiliado.NumeroEmpresa), Number(yoAfiliado.NumeroSucursal), function (res: Msg) {
                            var facturacion: FormaPagoEntity = res.Datos;
                ...
                get$facturacion$ObtenerFacturasCorpElectronicasXEmpresaSucursalBrokerLQ
                    (IDEmpresa, IDSucursal,'', function (res: Msg) {
                        var lstFacturas = new Array<FacturaCorporativa>();
                        lstFacturas = <Array<FacturaCorporativa>>res.Datos;
        ...
        if (index === 3) {
            if (tipoProceso === 'Independiente' || tipoProceso == 'Afiliado' || tipoProceso === 'Experience' || tipoProceso === 'Oncocare') {
                ...
                var filtro: FiltroSiniestralidadActual = new FiltroSiniestralidadActual();
                ...
                post$contrato$getOneByKey(filtro, function (res) {
                    var siniestralidadVigente = <SiniestralidadAcumulada>res;
    ...
    function SeleccionTipoPlan() {
        ...
        if (valorSeleccion != null) {
            ...
            if (tipoProceso === 'Independiente' || tipoProceso === 'Experience' || tipoProceso === 'Oncocare'){
                ...
                var filtro: ContratoEntityFilter = new ContratoEntityFilter();
                var brokers: Array<number> = new Array<number>();
                if (esGrupo) {
                ...
                post$contrato$ConsultarContratosIndividualesPorAgente(filtro, function (res) {                
                    var lst = <Array<ContratoEntityList>>res.Datos;
                    if (lst.length == 0)
                    ...
                    var TipoReporte = <number>$("#rb_formatoProducto").data('kendoButtonGroup').current().index();
                    if (TipoReporte == 0) {
                        $('#gridPlanesR').show();
                ...
                //pendiente empresa
                var filtroEmpres: ContratosIndividualesEmpresasPorAgenteFilter = new ContratosIndividualesEmpresasPorAgenteFilter();
                var brokers: Array<number> = new Array<number>();
                if (esGrupo) {
                    if (listasSeleccion == undefined || listasSeleccion.length == 0) {
                        alert('Debe selecccionar al menos un broker');
                ...
                post$contrato$ConsultarContratosCorpPorAgente(filtroEmpres, function (res) {
                    var lst = <Array<EmpresaList>>res.Datos;
                    if (lst.length == 0)
    ...
    $('#butCartas').click(function () {
        ...
        var filtro: FilterQpra = new FilterQpra();
        filtro.Contrato_Numero = Number(contratoCarta);
        var beneficiarios: Array<number> = new Array<number>();
    ...
    $('#btn_Buscar_Afiliado').click(function () {
        ...
        var brokerList = new Array<number>();
        if (esGrupo) {
            if (listasSeleccion == undefined || listasSeleccion.length == 0) {
                alert('Debe selecccionar al menos un broker');
                return false;
        ...
        filtro.SoloActivos = true;
        post$contrato$ConsultarContratosPorFiltroAfiliadoGeneral(filtro, function (result: Msg) {
            if (result == undefined) {
            ...
            var listaGeneral = new Array<ContratoEntityList>();
            ...
            $('#gridBusqueda').kendoGrid({
                ...
                columns: [
                    ...
                    //{ field: "pendientePago", title: "Pendiente Pago", width: "110px", headerAttributes: { style: "white-space: normal" }, filterable: { multi: true, search: true } }
    ...
    $('#btn_Buscar_Empresa').click(function () {
        var listEmpresas = new Array<EmpresaList>();
    ...
    $('#btn_Buscar_Factura').click(function () {
        ...
        var FiltroDesde = <Date>$('#inp_desde_facturar').data('kendoDatePicker').value();
        var FiltroHasta = <Date>$('#inp_hasta_facturar').data('kendoDatePicker').value();
        ...
        if (yoAfiliado.CodigoProducto != 'COR' && yoAfiliado.CodigoProducto != 'SMAR'
            && yoAfiliado.CodigoProducto != 'POOL') {
            get$facturacion$ConsultarFacturacionIndividualXCliente
                (IDNumeroContratoAfiliado, numFact, FiltroDesde, FiltroHasta, function (res: Msg) {
                    var lstRes = <Array<Facturacion_ReporteGeneral>>res.Datos;
                    var facturas = new Array<Facturacion_ReporteGeneral>();
                    for (let f of lstRes) {
            ...
            get$facturacion$ObtenerFacturasCorpElectronicasXEmpresaSucursalBrokerLQ
                (yoAfiliado.NumeroEmpresa, yoAfiliado.NumeroSucursal,numFact, function (res: Msg) {
                    var lstFacturas = new Array<FacturaCorporativa>();
                    lstFacturas = <Array<FacturaCorporativa>>res.Datos;
                    for (let i in lstFacturas) {
                        lstFacturas[i].FechaMes = formatDate(lstFacturas[i].FechaEmision);
    ...
    $('#btn_Buscar_Reclamo').click(function () {
        var FiltroDesde = <Date>$('#inp_desde_reclamo').data('kendoDatePicker').value();
        var FiltroHasta = <Date>$('#inp_hasta_reclamo').data('kendoDatePicker').value();
        if (FiltroDesde == null || FiltroHasta == null) {
            return alert('Debe seleccionar las fechas de inicio y fin para poder ejecutar el reporte');
        }
        if (FiltroDesde > FiltroHasta)
        ...
        post$liquidacion$ObtenerLiquidacionesIndividuales
            (filtroReclamos, function (res: Msg) {
                var Reclamo = <Array<LiquidacionIndividual>>res.Datos;
                if ($('#gridReclamos').data().kendoGrid) {
                    $('#gridReclamos').data().kendoGrid.destroy();
                    $('#gridReclamos').empty();
                }
    ...
    function detalleEventoClick2(e) {
        ...
        post$reclamos$generarPdf64(filtrador,
            function (result) {
    ...
    $('#btn_Buscar_autorizaciones').click(function () {
        var listaAutorizaciones = new Array<PreAutorizacionesDummy>();
    ...
    function seleccionarPlanInd(e) {        
        var dataItem = this.dataItem($(e.currentTarget).closest("tr"));
        var PlanInd: ContratoEntityList = dataItem;
    ...
    function seleccionarPlanEmp(e) {
        var dataItem = this.dataItem($(e.currentTarget).closest("tr"));
        var EmpresaFilter: EmpresaList = dataItem;
    ...
    function VerDetalleFactura(e) {
        var dataItem = this.dataItem($(e.currentTarget).closest("tr"));
        var facturaSeleccionada: facturaEmpresaDummy = dataItem;
    ...
    function VerDetalleAfiliadoEmpresa(id) {
        ...
        filtro.SoloActivos = true;
        post$contrato$ConsultarEmpresasPorAgente(filtro, function (result: Msg) {
            if (result == undefined) {
                return alert('Ha ocurrido un problema en la obtención de datos del servidor.');
            }
            var listaEmp=<Array<EmpresaList>>result.Datos;
            ...
            var pendienteP: ConsultaFacturacionFilter = new ConsultaFacturacionFilter();
            ...
            post$facturacion$ConsultarFacturacionPendienteEmpresa(pendienteP, function (result: Msg) {
            ...
            var fechaVigencia: Date = new Date(yoEmpresa[0].FechaInicioSucursalDate);
    ...
    function VerDetalleAfiliado(id) {
        ...
        filtro.NumeroContrato = yoAfiliado.NumeroContrato;
        filtro.SoloActivos = true;
        post$contrato$ConsultarContratosPorFiltro(filtro, function (result: Msg) {
            if (result == undefined) {
                return alert('Ha ocurrido un problema en la obtención de datos del servidor.');
            }
            var listaAfiliado = <Array<ContratoEntityList>>result.Datos;
            ...
            post$facturacion$ConsultarFacturacionPendienteAfiliado(pendienteP, function (result: Msg) {
                if (result == undefined || result == null) {
            ...
            if (tipoProceso === 'Independiente' || tipoProceso === 'Afiliado' || tipoProceso === 'Experience' || tipoProceso === 'Oncocare') {
                ...
                get$contrato$ObtenerBeneficiariosIndividuales(yoAfiliado.NumeroContrato, function (res: Msg) {
                    var numeroBeneficiario = <Array<BeneficiarioEntity>>res.Datos;
                    ...
                    if (numeroBeneficiario.length > 0) {
                        ...
                        var beneficiarioCompleto = new Array<BeneficiarioEntity>();
                        ...
                        BeneficiariosIndividuales = new Array<BeneficiarioEntity>();
                    }
                },
                    function () { });
                //mostramos la grilla
    ...
    function LlenarFiltro() {
        ...
        listasSeleccion = new Array<GrupoCorporativo>();
        for (let r of sl) {
    ...
    function GenerarReporte() {
        ...
        var empresas = new Array<string>();
        var listas = new Array<string>();
        var productos = new Array<string>();
        ...
        if (TipoReporte == 0) {
            ...
            get$contrato$ObtenerMovimientosBeneficiariosEmpresas(empresas, listas, uniqueItems, FiltroDesde, FiltroHasta, estados,
                function (res: Msg) {
                    var lst = <MovimientoBeneficiarioEmpresa>res.Datos;
                    respGeneral = lst;
        ...
        if (TipoReporte == 1) {
            ...
            get$contrato$ObtenerMovimientosBeneficiariosEmpresas(empresas, listas, uniqueItems, FiltroDesde, FiltroHasta, estados,
                function (res: Msg) {
                    var lst = <ReclamoEmpresaLista>res.Datos;
                    respGeneral = lst;
                    // cargo la ruta en el reporteador
    ...
    function GenerarReporteLiquidacion() {
        var FiltroDesde = <Date>$('#dt_DesdeLiquidacion').data('kendoDatePicker').value();
        var FiltroHasta = <Date>$('#dt_HastaLiquidacion').data('kendoDatePicker').value();
        ...
        var empresas = new Array<string>();
        var listas = new Array<string>();
        var listasElejidas = new Array<ListaElegida>();

        empresas.push('' + IDEmpresa);
        listas.push('' + IDSucursal);
       
        var TipoReporte = <number>$("#rb_formato2").data('kendoButtonGroup').current().index();
        // 0 = pantalla, 1 = exportable
        ...
        if (TipoReporte == 0) {
            ...
            get$Liquidacion$ObtenerReclamosGrupoEmpresaListaBeneficiario(empresas, listas, FiltroDesde, FiltroHasta, $('#txt_IdentificacionLiquidacion').val(),
                function (res: Msg) {
                    $('#pnl_ResultadoWebLiquidacion').show();
                    $('#pnl_ResultadoReporteadorLiquidacion').hide();
                    var lst = <Array<ReclamoEmpresaLista>>res.Datos;
                    //PROCESO LOS RESULTADOS PARA OBTENER EL SEXO Y LA EDAD
                    var finallst = new Array<ReclamoEmpresaLista>();
                    for (let r of lst) {
    ...
    function GenerarReporteCopago() {
        var FiltroDesde = <Date>$('#dt_DesdeCopagos').data('kendoDatePicker').value();
        var FiltroHasta = <Date>$('#dt_HastaCopagos').data('kendoDatePicker').value();
        ...
        var empresas = new Array<string>();
        var listas = new Array<string>();
        var empresasn = new Array<number>();
        var consolidado = new Array<numerosSucursalEmpresa>();
        var ConsolidadoFinal = new Array<numerosSucursalEmpresa>();
        ...
        var TipoReporte = <number>$("#rb_formatoCopagos").data('kendoButtonGroup').current().index();
        // 0 = pantalla, 1 = exportable
        ...
        if (TipoReporte == 0) {
            ...
            post$Copagos$ConsultaCopagos(1, consolidado, FiltroDesde, FiltroHasta, estadoFacturas, $('#txt_IdentificacionCopagos').val(),
                function (res: Msg) {
                    ...
                    var listaCompleta = new Array<CopagoPendiente>();
                    var lstRes = <Array<CopagoPendiente>>res.Datos;
    ...
    function GenerarReporteAutorizacionesR() {
        var FiltroDesde = <Date>$('#dt_DesdeAutorizacionesR').data('kendoDatePicker').value();
        var FiltroHasta = <Date>$('#dt_HastaAutorizacionesR').data('kendoDatePicker').value();
        ...
        var TipoReporte = <number>$("#rb_formatoAutorizacionesR").data('kendoButtonGroup').current().index();
        // 0 = pantalla, 1 = exportable
        ...
        if (TipoReporte == 0) {
            ...
            post$CorredoresAutorizacionController$ConsultaAutorizacionesEspecificas(1, ConsolidadoFinal, FiltroDesde, FiltroHasta, $('#txt_IdentificacionAutorizacionesR').val(),
                function (res: Msg) {
                    // Muestro el panel de la grilla
                    $('#pnl_ResultadoWebAutorizacionesR').show();
                    $('#pnl_ResultadoReporteadorAutorizacionesR').hide();
                    // una vez que traemos todos los datos vamos a filtrar cada resultado por el numero de listas seleccionadas
                    var listaCompleta = new Array<AutorizacionesCubiertas>();
                    var lstRes = <Array<AutorizacionesCubiertas>>res.Datos;
                    for (let suc of listas) {
                        var filtrados = lstRes.filter(function (s) { return s.SucursalEmpresa == Number(suc) });
                        for (let sucRest of filtrados) {
    ...
    function GenerarReporteSiniestralidad() {
        var FiltroDesde = <Date>$('#dt_DesdeSiniestralidad').data('kendoDatePicker').value();
        var FiltroHasta = <Date>$('#dt_HastaSiniestralidad').data('kendoDatePicker').value();
        $('#pnl_ResultadoReporteadorSiniestralidad').hide();
        if (FiltroDesde == null || FiltroHasta == null) {
            $("#tipoReporteSiniestralida").data('kendoDropDownList').value(null);
            return alert('Debe seleccionar las fechas de inicio y fin para poder ejecutar el reporte');
        ...
        if (tipoReporteSiniestralidad == "Siniestralida") {
            ...
            $('#pnl_ResultadoReporteadorSiniestralidad').show();            
            consolidado = new Array<numerosSucursalEmpresa>();
            for (let emp of uniqueItems) {
            ...
            post$siniestralidad$Mensualizada(FiltroDesde, FiltroHasta, consolidado, tipoProducto,
                function (res: Msg) {
                    var listIDNR = new Array<SiniestralidadLista>();
                    var lstRes = <Array<SiniestralidadLista>>res.Datos;
                    var listas:Array <number>= new Array<number>()
                    ...
                    for (let listaIndividual of unique) {
                        ...
                        for (let sucRest of filtrados) {//filtro y agrupo por sucursarl y a cada sucursal por cada mes le hag le calculo IDNR
                            sucRest.LiquidacionesSinIdnr = sucRest.Liquidaciones
                            if (contadorMeses != 0)//luego del primer mes se multiplica por 0,075
        ...
        if (tipoReporteSiniestralidad == "SiniestralidadTrabajo") {
            ...
            post$siniestralidad$Mensualizada(FiltroDesde, FiltroHasta, consolidado, tipoProducto,
                function (res: Msg) {
                    var listaCompleta = new Array<SiniestralidadLista>();
                    var listIDNR = new Array<SiniestralidadLista>();
                    var lstRes = <Array<SiniestralidadLista>>res.Datos;
                    for (let suc of listas) {
                    ...
                    post$facturacion$GenerarExportableSiniestralidadLista(listIDNR,
                        function (res: Msg) {
        ...
        if (tipoReporteSiniestralidad == "SiniestralidadLista") {
            ...
            $('#pnl_ResultadoReporteadorSiniestralidad').show();
            post$siniestralidad$Mensualizada(FiltroDesde, FiltroHasta, ConsolidadoFinal, tipoProducto,
                function (res: Msg) {
                    // una vez que traemos todos los datos vamos a filtrar cada resultado por el numero de listas seleccionadas
                    var listaCompleta = new Array<SiniestralidadLista>();
                    var listIDNR = new Array<SiniestralidadLista>();
...
function DescargarXML(e) {
    e.preventDefault();
    var dataItem = <FactCorpElectronica>this.dataItem($(e.currentTarget).closest("tr"));

    get$ebilling$ConsultaFacturaXml(dataItem.NumeroFactura,
        function (res) {
            //convert base64 string to byte array
...
function VerCuota(e) {
    e.preventDefault();
    var dataItem = <Facturacion_ReporteGeneral>this.dataItem($(e.currentTarget).closest("tr"));
...
function DescargarXMLIndv(e) {
    ...
    get$ebilling$ConsultaFacturaXml(facturaInd,
        function (res) {
            //convert base64 string to byte array
...
function DescargarRIDE(e) {
    e.preventDefault();
    var dataItem = <FacturaCorporativa>this.dataItem($(e.currentTarget).closest("tr"));

    get$ebilling$ConsultaFacturaPdf(dataItem.NumeroFactura,
        function (res) {

            var respuesta = <wsConsultaDocumentoRespuesta>res;
...
function DescargarRIDEIndividuales(e) {
    ...
    var facturaInd = CodigoSucursal + '-' + CodigoSerie + '-' + CodigoFactura;
    get$ebilling$ConsultaFacturaPdf(facturaInd,
        function (res) {
...
function DescargarSoportes(e) {
    ...
    get$facturacion$ObtenerDatosReporteSoportesFactura(dataItem.IDEmpresa, dataItem.IDSucursal, dataItem.IDCuota, 1,
...
function DescargarSoporteCarta(e) {
    ...
    e.preventDefault();
    var dataItem = <QpraEntity>this.dataItem($(e.currentTarget).closest("tr"));

    get$Qpra$CorredoresObtenerArchivoQpra(dataItem.Id_DetalleArchivo,
        ...
        function (res: Msg) {
            ...
            var carta = <QpraArchivo>res.Datos;
...
function DescargarSoportesExcel(e) {
    ...
    get$facturacion$ObtenerDatosReporteSoportesFactura(dataItem.IDEmpresa, dataItem.IDSucursal, dataItem.IDCuota, 2,
