var cambioList: Array<CambioPlan>;
var FechaCarencia: Date;
head.ready(function () {
    ...
    $('#btn_Buscar_Cliente_Cambio').click(function () {
        ...
        if (cedulaCambio.trim() !== '') {
            ...
            get$portalcorporativo$ObtenerClientesEmpresaIdentificacion(
                idEmpresa, cedulaCambio.trim(), Esactivo, function (result) {
                    //console.log(result);
                    if (result != null) {
                        lstPersonas = <Array<Persona>>result;
        ...
        if (nombreCambio.trim() !== '') {
            var idEmpresa = UsuarioLogueado.IdEmpresa;
            post$portalcorporativo$ObtenerClientesEmpresa(
                idEmpresa, nombreCambio.trim(), Esactivo, function (result) {
                    console.log(result);
                    if (result != null) {
                        lstPersonas = <Array<Persona>>result;
                        if (lstPersonas.length > 0) {
        ...
        function verUsuarioCambio(e) {
            ...
            personaActivacion = <Persona>dataItem;
