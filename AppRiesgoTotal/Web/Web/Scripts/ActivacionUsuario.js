...
head.ready(function () {
    ...
    function CambioProvincia() {
        ...
        post$divisionpolitica$ObtenerCiudadesPorIdProvincia(IdProvincia, function (result) {
    ...
    $('#btn_AgregarDependiente').click(function () {
        ...
        dep.Identificacion = $('#txt_Identificacion').val();
        dep.Parentesco = parseInt($('#txt_Parentesco').val());
        dep.TipoIdentificacion = $("#txt_TipoIdentificacion").data('kendoDropDownList').value();
        dep.Anterior = false;
        ...
        get$beneficiario$ObtenerSiYaExisteBeneficiario(Infobase.inclusiones[0].EmpresaID, Infobase.inclusiones[0].SucursalID, $("#txt_Identificacion").val(), function (res) {
            if (res.Datos == true) {
            ...
            if (dep.TipoIdentificacion == "1") {
                // primero llamo al registro civil para ver si lo encuentra
                post$registrocivil$ObtenerPersonaPorNumeroIdentificacion($('#txt_Identificacion').val(), function (result) {
                    ...
                    if (lstDependientes.filter(function (e) { return e.Identificacion == dep.Identificacion; }).length > 0) {
                    ...
                    if (dep.Parentesco === 2 && lstDependientes.filter(function (e) { return e.Parentesco === 2; }).length > 0) {
                    ...
                    if (Cobertura == "AT")
                        return alert('En una tarifa de afiliado solo no puede agregar beneficiarios');
                    if (Cobertura == "A1" && lstDependientes.length == 1)
                        return alert('En una tarifa afiliado más uno, no se permite agregar más de un beneficiario');
            ...
            else if (dep.TipoIdentificacion == "2") {
                dep.Apellidos = $('#txt_Apellidos').val().toUpperCase();
                dep.FechaNacimiento = $('#txt_FechaNacimiento').data('kendoDatePicker').value();
                dep.Nombres = $('#txt_Nombres').val().toUpperCase();
    ...
    $('#btn_Grabar').click(function () {
        ...
        if ($('#txt_Provincia').data('kendoDropDownList').value() == "0" || $('#txt_Provincia').data('kendoDropDownList').value() == "" || $('#txt_Provincia').data('kendoDropDownList').value() == null)
            msgErrores += "No se ha llenado el campo de la provincia. \n";
        if ($('#txt_Ciudad').data('kendoDropDownList').value() == "0" || $('#txt_Ciudad').data('kendoDropDownList').value() == "" || $('#txt_Ciudad').data('kendoDropDownList').value() == null)
            msgErrores += "No se ha llenado el campo de la ciudad. \n";
        if ($('#txt_Sector').val() == "")
            msgErrores += "No se ha llenado el campo del sector \n";
        if ($('#txt_DireccionDomicilio').val() == "")
            msgErrores += "No se ha llenado el campo de la dirección del domicilio \n";
        if ($('#txt_Email').val() == "")
            msgErrores += "No se ha llenado el campo del Email Personal \n";
        if (!validateEmail($('#txt_Email').val()))
            msgErrores += "El correo electrónico no cumple el formato esperado \n";
        if ($('#txt_TelefonoDomicilio').val() == "" || GetRawfromMasked('txt_TelefonoDomicilio') == "")
            msgErrores += "No se ha llenado el campo del Teléfono del Domicilio \n";
        if ($('#txt_TelefonoCelular').val() == "")
            msgErrores += "No se ha llenado el campo del Teléfono Celular \n";
        if ($('#cmb_Banco').data('kendoDropDownList').value() == "0" || $('#cmb_Banco').data('kendoDropDownList').value() == "" || $('#cmb_Banco').data('kendoDropDownList').value() == null)
            msgErrores += "No se ha llenado el campo del Banco. \n";
        if ($('#cmb_TipoCuenta').data('kendoDropDownList').value() == "0" || $('#cmb_TipoCuenta').data('kendoDropDownList').value() == "" || $('#cmb_TipoCuenta').data('kendoDropDownList').value() == null)
            msgErrores += "No se ha llenado el campo del Tipo de Cuenta. \n";
        if ($('#txt_NumeroCuenta').val() == "")
            msgErrores += "No se ha llenado el campo del Número de Cuenta \n";
        if (GetRawfromMasked('txt_TelefonoDomicilio').length != 9)
            msgErrores += "El valor del teléfono de Domicilio está incompleto. Debe llenar el número con el código regional. \n";
        if (GetRawfromMasked('txt_TelefonoCelular').length != 10)
            msgErrores += "El valor del teléfono de Celular está incompleto. Debe llenar el número con el código regional. \n";
        ...
        if (Infobase.inclusiones[0].CompletadoEnrolamiento == false) {
            ...
            if (Cobertura == "AT" && lstDependientes.length > 0)
                msgErrores += "No se puede agregar beneficiarios a un plan afiliado solo";
            if (Cobertura == "A1" && lstDependientes.length > 1)
                msgErrores += "No se puede agregar más de un beneficiario en un plan de afiliado más uno";
            if (Cobertura == "AF" && lstDependientes.length == 0)
                msgErrores += "El afiliado ha elegido el plan de afiliado más familia, sin embargo no ha llenado sus beneficiarios.";
        }
        // si hay errores
        if (msgErrores != "") {
        ...
        if (Cobertura == "A1" && lstDependientes.length == 0) {
            Confirmation("El plan contratado permite agregar un beneficiario, pero no se lo ha agregado en pantalla. Desea continuar sin agregarlo?", function () {
                ProcederGrabacion();
            }, function () {
                // alert('no');
            });
            ...
            Confirmation("El plan contratado permite agregar varios beneficiarios, pero no se ha agregado ninguno en pantalla. Desea continuar sin agregarlos?", function () {
                ProcederGrabacion();
            }, function () {
                //alert('no');
            });
            ...
            Confirmation("El plan contratado permite dar cobertura a todas las personas de la familia, pero se ha agregado a una sola. Desea continuar de esta manera?", function () {
                ProcederGrabacion();
            }, function () {
                //alert('no');
            });
    ...
    function ProcederGrabacion() {
        Confirmation("Está seguro de proceder el registro de los datos ingresados?", function () {
            ...
            if (Infobase.inclusiones[0].CompletadoEnrolamiento != true) {
                if (Infobase.sucursal.Configuracion != undefined && Infobase.sucursal.Configuracion != null && Infobase.sucursal.Configuracion != '') {
                    var subSucursales = JSON.parse(Infobase.sucursal.Configuracion);
                    if (subSucursales != undefined && subSucursales != null) {
                        ...
                        for (let i in dCOB) {
                            ...
                            if ($('#chk_' + id).data('kendoMobileSwitch').check()) {
                                ...
                                inc.Resultados = new Array(); // no debería copiarse de la inclusión inicial
                ...
                for (item in lstDependientes) {
                    ...
                    if (dependiente.Anterior == false) {
                        ...
                        if (actualizacion.Dependientes == undefined)
                            actualizacion.Dependientes = new Array();
                        ...
                        if (actualizacion.Beneficiarios == undefined)
                            actualizacion.Beneficiarios = new Array();
            ...
            if (actualizacion.Beneficiarios == undefined)
                actualizacion.Beneficiarios = new Array();
            post$portalcorporativo$GuardarInformacionCliente(actualizacion, function (result) {
                if (result != null) {
                    var message = "";
                    var error = false;
                    for (let i in result) {
    ...
    post$transaccion$getDatosPI(function (result) {
        ...
        post$divisionpolitica$ObtenerProvincias(function (result) {
            ...
            try {
                ...
                // LECTURA DE DATOS
                get$masivo$ObtenerDatosEnrolamientoPorID(IdRegistro, function (result) {
                    ...
                    if (data.inclusiones[0].CompletadoEnrolamiento == true) {
                        ...
                        if (data.sucursal.Configuracion != null && data.sucursal.Configuracion != undefined) {
                            ...
                            var subSucursales = JSON.parse(data.sucursal.Configuracion);
                            if (subSucursales != undefined && subSucursales != null) {
...
//# sourceMappingURL=ActivacionUsuario.js.map