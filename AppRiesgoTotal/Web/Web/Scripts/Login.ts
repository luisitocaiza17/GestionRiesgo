...
head.ready(function () {
    ...
    $('#btn_Continuar').click(function () {
        ...
        get$CorredoresAgenteVenta$CorredoresObtenerAgentesVentaPorRuc(ruc, function (res: Msg) {
            var lstRes = <Array<AgenteVentaCorredoresEntity>>res.Datos;
    ...
    var UsuarioLogueado: PC_UsuarioRol_Result = null;

    function Ingresar() {

        var usr: PC_UsuarioRol_Result = new PC_UsuarioRol_Result();
        usr.NombreUsuario = $('#txt_username').val().toString();
        usr.Contrasena = $('#txt_password').val().toString();

        post$usuario$CorredoresObtenerUsuarioPorNombreyClave(usr, function (result: Msg) {
            if (result == undefined) {
                return alert('Ha ocurrido un problema en la obtención de datos del servidor.');
            }
            UsuarioLogueado = <PC_UsuarioRol_Result>result.Datos;

            //verificacion de usuario administrador
            if (UsuarioLogueado.IdCorredor == 1) { // si es un usuario de salud

                get$CorredoresAgenteVenta$CorredoresObtenerAgentesVentaListSQL(
                    function (res: Msg) {
                        var lst = <Array<PC_AgenteVenta>>res.Datos;
    ...
    function CambioBroker() {
        ...
        get$usuario$CorredoresObtenerUsuariosPorCorredorList(IdCorredor,
            function (res: Msg) {

                var data = <Array<PC_UsuarioRol_Result>>res.Datos;
    ...
    $('#btn_IngresarAdmin').click(function () {
        ...
        UsuarioLogueado = <PC_UsuarioRol_Result>dataItem;
        CompletarIngreso();
    });

    var TerminosCondicionesIDActual: number = 0;
    function CompletarIngreso() {

        get$terminoscondiciones$CorredoresTerminosCondicionesIdActual(
            function (res: Msg) {
                TerminosCondicionesIDActual = <number>res.Datos;
                ...
                if (UsuarioLogueado.TerminosCondicionesAprobado == null || UsuarioLogueado.TerminosCondicionesAprobado == undefined || 
                    UsuarioLogueado.TerminosCondicionesAprobado < TerminosCondicionesIDActual) { // valida si no ha aceptado antes o lo aceptado antes es menor a lo actual

                    // presenta la ventana para pedirle que acepte las ultimas condiciones

                    get$terminoscondiciones$CorredoresTerminosCondicionesPorID(TerminosCondicionesIDActual,
                        function (res: Msg) {
                            var tc = <PC_TerminosCondiciones>res.Datos;
    ...
    function TerminarInicioSesion() {
        // Carga los permisos
        get$usuario$CorredoresObtenerPermisosUsuarioPorID(UsuarioLogueado.Id,
            function (res: Msg) {

                var permisos = <Array<PC_Permiso_Result>>res.Datos;
    ...
    $('#btn_Recuperar').click(function () {
        ...
        get$usuario$CorredoresResetearClaveCorredores(Number(brokerId), $('#txt_nombreUsuario').val(),
            function (result) {
    ...
    $('#btn_DescargarTerminos').click(function () {
        get$terminoscondiciones$CorredoresTerminosCondicionesPDFContenido(TerminosCondicionesIDActual,
            function (res: Msg) {
    ...
    $('#btn_AceptarTC').click(function () {
        ...
        get$terminoscondiciones$CorredoresTerminosCondicionesAceptado(TerminosCondicionesIDActual, UsuarioLogueado.Id,
            function (res: Msg) {
                if (res.Datos == true) {
